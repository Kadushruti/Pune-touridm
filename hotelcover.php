                    ...
                    <div class="content-header row align-items-center m-0">
                        <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
                            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Hotel Cover Image</li>
                            </ol>
                        </nav>
                                    ...
                                    <div class="card-body text-center">
                                        <?php
                                         if (!empty($row['coverimage'])) {
                                             $img = "uploads/hotels/coverimage/".$row['coverimage'];
                                         }
                                         else{
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Update Hotel Cover Image</h5>
...
{
    $targetDir = "uploads/hotels/coverimage/";
    ...
    $allowTypes = array('jpg','png','jpeg');
        if(in_array($fileType, $allowTypes)){
            if(move_uploaded_file($_FILES["coverimage"]["tmp_name"], $targetFilePath)){
                $id = $_GET['hotelcover'];
                if (!empty($row['coverimage'])) {
                    $deleteImagePath = "uploads/hotels/coverimage/".$row['coverimage'];
                    unlink($deleteImagePath);
                }
                $uploadcover = $conn->query("UPDATE tbl_hotels SET coverimage ='$fileName' WHERE id='$id'");
                if($uploadcover){
                                ?>
