                    ...
                    <div class="content-header row align-items-center m-0">
                        <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
                            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Room Cover Image</li>
                            </ol>
                        </nav>
                                    ...
                                    <div class="card-body text-center">
                                        <?php
                                         if (!empty($row['coverimage'])) {
                                             $img = "uploads/rooms/coverimage/".$row['coverimage'];
                                         }
                                         else{
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Update Room Cover Image</h5>
...
{
    $targetDir = "uploads/rooms/coverimage/";
    ...
    $allowTypes = array('jpg','png','jpeg');
        if(in_array($fileType, $allowTypes)){
            if(move_uploaded_file($_FILES["coverimage"]["tmp_name"], $targetFilePath)){
                $id = $_GET['roomcover'];
                if (!empty($row['coverimage'])) {
                    $deleteImagePath = "uploads/rooms/coverimage/".$row['coverimage'];
                    unlink($deleteImagePath);
                }
                $uploadcover = $conn->query("UPDATE tbl_rooms SET coverimage ='$fileName' WHERE id='$id'");
                if($uploadcover){
                                ?>
