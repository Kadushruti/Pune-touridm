...
<?php 
if (isset($_GET['hotelgallery'])) {
    $id = $_GET['hotelgallery'];
        }
?>
    <body class="fixed">
        <!-- Page Loader -->
                    ...
                    <div class="content-header row align-items-center m-0">
                        <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
                            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Hotel Image Gallary</li>
                            </ol>
                        </nav>
                            ...
                            <div class="col-md-12 col-lg-12">
                                <div class="card mb-4">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                        <?php
                                        $sql = "SELECT * FROM tbl_hotels_gallery  WHERE hotel_id ='".$id."' ORDER BY gallery_id DESC";
                                        $result = mysqli_query($conn,$sql);
                                        if(mysqli_num_rows($result) > 0)
                                        {
                                            while($row = mysqli_fetch_array($result))
                                            {
                                                $imageURL = 'uploads/hotels/gallery/'.$row["file_name"];
                                        ?>
...
{
    var hotelgallery = <?php echo $_GET['hotelgallery']?>;
   ...
   function(){ 
       window.location = 'hotelgallery.php?deleteimage='+deleteimage+'&&hotelgallery='+hotelgallery;
   }
);   
}
</script>
...
{
    $id = $_GET['deleteimage'];
    $sql1 = "DELETE FROM tbl_hotels_gallery WHERE gallery_id=$id";
    ...
    if($success1)
    {
        ...
        <script type="text/javascript">
            ...
            swal({title: "Success", text: "Image Delete Successfully..!", type: "success"},
               function(){ 
                   window.location = "hotelgallery.php?hotelgallery="+hotelgallery;
               }
            );
        </script>
<?php
...
if(isset($_POST['submit'])){ 
    $targetDir = "uploads/hotels/gallery/"; 
    ...
    if(!empty($fileNames)){ 
        foreach($_FILES['files']['name'] as $key=>$val){ 
            ...
            if(in_array($fileType, $allowTypes)){ 
                if(move_uploaded_file($_FILES["files"]["tmp_name"][$key], $targetFilePath)){  
                    $insertValuesSQL .= "('".$fileName."','".$_GET['hotelgallery']."'),"; 
                }else{ 
        ...
        if(!empty($insertValuesSQL)){ 
            ...
            $sql = "INSERT INTO tbl_hotels_gallery (file_name,hotel_id) VALUES $insertValuesSQL";
